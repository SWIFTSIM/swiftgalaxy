...
class TestRecenteredVisualisation:
    ...
    @pytest.mark.skip(reason="ray trace not documented (not fully implemented?)")
    @pytest.mark.parametrize("z_slice", [False, True])
    def test_recentered_ray_trace(self, sg, sg_autorecentre_off, z_slice):
        """
        We should be able to make the same ray trace whether we recentered or not.
        """
        z_limits = [1.5, 2.5] if z_slice else []
        recentered_z_limits = [-0.5, 0.5] if z_slice else []
        kwargs = {
            "resolution": 4,
            "panels": 4,
            "project": "masses",
