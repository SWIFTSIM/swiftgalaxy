...
class TestVelociraptorWithSWIFTGalaxy:
    ...
    @pytest.mark.parametrize("centre_type", ("", "minpot", "mbp", "_gas", "_star"))
    def test_centre_in_particles(self, web_examples_tmpdir, centre_type):
        """
        Check that centres are inside the particle distribution.

        We use data actually generated by velociraptor instead of our "by hand" dummy
        dataset for this to check for comoving/physical issues.
        """
        vr = Velociraptor(
            web_examples_tmpdir.velociraptor,
            halo_index=3,
            centre_type=centre_type,
        )
        assert vr.scale_factor < 1
        sg_uncentered = SWIFTGalaxy(
            web_examples_tmpdir.snapshot, vr, auto_recentre=False
        )
        xyz_min = (
            sg_uncentered.dark_matter.coordinates.min(axis=0)
            % sg_uncentered.metadata.boxsize
        )
        xyz_max = (
            sg_uncentered.dark_matter.coordinates.max(axis=0)
            % sg_uncentered.metadata.boxsize
        )
        sg = SWIFTGalaxy(web_examples_tmpdir.snapshot, vr)
        assert (sg.centre < xyz_max).all()
        assert (sg.centre > xyz_min).all()
